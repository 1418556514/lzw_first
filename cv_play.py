...
def model_train(ori_model, epoch_num, optimizer, data_loaders, criterion, dtest_data_loaders=None,
    ...
    for epoch in range(epoch_num):
        ...
        if dtest_data_loaders and epoch>5:
...
# 引入下面这个是防止cv把扩增后的数据分别划分到训练和测试集，造成数据泄露
split_example=pd.read_csv('all_data.csv')
all_p_data = pd.read_csv('more_process_data_0.2_new.csv')
split_p_res=kf.split(split_example)
all_n_data = pd.read_csv('more_process_n_data_0.05.csv')
split_n_res=kf.split(all_n_data)
for i in range(5):
    p_tem=split_p_res.__next__()
    column_p_train =[]
    for j in p_tem[0]:
        column_p_train+=list(range(j*50,(j+1)*50))
    data_p_train=all_p_data.iloc[column_p_train]
    column_p_test = []
    for j in p_tem[1]:
        column_p_test += list(range(j * 50, (j + 1) * 50))
    data_p_test = all_p_data.iloc[column_p_test]

    n_tem=split_n_res.__next__()
    data_n_train=all_n_data.iloc[n_tem[0]]
    data_n_test=all_n_data.iloc[n_tem[1]]

    need_col = ['miRNA_seq', 'gene_seq', 'result']
    train_data = pd.concat([data_p_train[need_col], data_n_train[need_col]]).reset_index(drop=True)
    test_data = pd.concat([data_p_test[need_col], data_n_test[need_col]]).reset_index(drop=True)
    ...
    model_dic, train_acc_dic, train_loss_dic, test_acc_dic, test_loss_dic = \
        model_train(tp_resnet_model, 8, ori_optimizer, train_data_loaders, train_criterion,test_data_loaders, extra_model_name=name)
